...
public class KafkaStreamPublisher implements MessagePublisher {
  ...
  public boolean kafkaPublish(String payload, String partitionKey) {
    try {
      ...
      Message<String> msg =
          MessageBuilder.withPayload(payload).setHeader("partitionKey", partitionKey).build();

      boolean sent = streamBridge.send(kafkaBinding, msg);

      if (sent) {
        log.info(
            "【KafkaPublisher】Message sent to Kafka successfully. partitionKey={}", partitionKey);
      } else {
